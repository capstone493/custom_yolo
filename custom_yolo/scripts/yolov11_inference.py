...
class YOLOv11ROS:
    def __init__(self):
        # Initialize ROS node
        rospy.init_node('yolov11_ros_node', anonymous=True)

        # Parameters
        self.image_topic = rospy.get_param('~image_topic', '/iris_depth_camera_0/camera/rgb/image_raw')
        self.confidence_threshold = rospy.get_param('~confidence_threshold', 0.5)
        self.result_topic = rospy.get_param('~result_topic', '/yolov11/detections')

        # Initialize YOLOv11 model
        self.model = YOLO('yolo11n.pt')  # Load YOLOv11 model (use 'yolov11s.pt', 'yolov11m.pt', etc. as needed)
        ...
        rospy.loginfo("YOLOv11 ROS node initialized. Subscribing to: {}".format(self.image_topic))
...
if __name__ == '__main__':
    try:
        yolov11_ros = YOLOv11ROS()
        rospy.spin()
    except rospy.ROSInterruptException:
        rospy.logerr("YOLOv11 ROS node interrupted.")
